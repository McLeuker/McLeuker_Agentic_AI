"""
Computer Use Agent — kimi-2.5 Model Computer Use Capability
===========================================================

Enables the AI to control a virtual computer environment through:
- Screenshot capture and analysis
- Mouse control (move, click, scroll)
- Keyboard input (typing, shortcuts)
- GUI element detection and interaction

This implements the computer use capability similar to kimi-2.5's native feature,
enabling true end-to-end task execution on any software interface.
"""
...
# Try to import Playwright for browser/computer control
try:
...
class ComputerAction:
    ...
    action_type: str  # "mouse", "keyboard", "screenshot", "wait"
...
class ActionResult:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "screenshot": self.screenshot[:100] + "..." if self.screenshot else None,
            "error": self.error,
...
@dataclass
class GUIElement:
    """Detected GUI element on screen."""
    element_type: str  # "button", "input", "link", "text", "image", etc.
    text: Optional[str] = None
    coordinates: Tuple[int, int] = (0, 0)  # center x, y
...
class ComputerUseAgent:
    ...
    - Real-time streaming of actions
    
    Usage:
        agent = ComputerUseAgent(kimi_client)
        await agent.initialize()
        async for event in agent.execute_task("Open Chrome and search for AI news"):
            print(event)
    """
    ...
    async def execute_task(
        ...
        for step in range(max_steps):
            try:
                ...
                yield {"type": "screenshot", "data": {"step": step, "screenshot": screenshot_b64}}
                ...
                if result.screenshot:
                    yield {"type": "screenshot", "data": {"step": step, "after_action": True, "screenshot": result.screenshot}}
    ...
    async def _capture_screenshot(self) -> str:
        ...
        self._screenshot_history.append(b64_string)
    ...
    async def _get_next_action(
        ...
        """
        # Build conversation with screenshot
        messages = [
            {
                "role": "system",
...
Available actions:
1. mouse_move {"x": int, "y": int} - Move mouse to coordinates
2. mouse_click {"x": int, "y": int, "button": "left"|"right"} - Click at coordinates
3. mouse_double_click {"x": int, "y": int} - Double click at coordinates
4. mouse_scroll {"x": int, "y": int, "scroll_amount": int} - Scroll at position
5. keyboard_type {"text": str} - Type text
6. keyboard_press {"key": str} - Press a key (Enter, Tab, Escape, etc.)
7. keyboard_hotkey {"keys": [str]} - Press key combination (e.g., ["ctrl", "a"])
8. wait {"duration": float} - Wait for specified seconds
9. complete {} - Task is complete
...
Coordinates are in pixels from top-left (0,0)."""
    ...
    async def _is_task_complete(self, task: str) -> bool:
        ...
        # If all recent actions were successful and we're waiting, task might be complete
    ...
    async def find_elements_by_text(self, text: str) -> List[Dict]:
        """Find elements containing specific text."""
        if not self._page:
            return []
        try:
            # Use XPath to find elements by text
            elements = await self._page.query_selector_all(f"xpath=//*[contains(text(), '{text}')]")
            results = []
            for el in elements:
...
def get_computer_use_agent(llm_client: openai.AsyncOpenAI = None) -> ComputerUseAgent:
